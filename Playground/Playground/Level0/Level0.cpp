...
void Level0::on_create() {
	...
	m_camera->set_relative_position(glm::vec3(0.0f, 2.0f, 6.0f));
...
void Level0::on_update(alc::timestep ts) {
	...
	if (right != left) {
		...
		auto rot = m_meshRenderer->get_rotation();
		rot *= glm::quat(glm::vec3(0.0f, rotation, 0.0f));
		m_meshRenderer->set_rotation(rot);
	}
