...
namespace alc {
	namespace gfx3d {
		...
		scenegraph3d::shader_group* scenegraph3d::get_shader_group(shader s) {
			// return the default shader group
			if (!s.is_valid()) return &(s_shaderGroups.front());
		...
		scenegraph3d::mesh_group* scenegraph3d::get_mesh_group(shader_group* group, mesh m) {
			if (!m.is_valid()) return nullptr;
			// find mesh group
		...
		void scenegraph3d::__init(const engine_settings* set) {
			...
			if (set->renderer3d.defaultShader != "")
				s_defaultShader = shader::loadsource(set->renderer3d.defaultShader);
			if (!s_defaultShader.is_valid())
				s_defaultShader = shader::loadsource(c_basic_mesh_shader);
		...
		void scenegraph3d::__exit() {
			...
			if (s_defaultShader) shader::unload(s_defaultShader);
		...
		//void scenegraph3d::___init(const engine_settings* set) {
		//	// load default shader
		//	if (set->renderer3d.defaultShader != "") 
		//		s_defaultShader = shader::loadsource(set->renderer3d.defaultShader);
		//	if (!s_defaultShader.is_valid()) 
		...
		//				for (auto* meshComponent : meshGroups.dynamicMeshes) {
		//
		//					/////// TODO: set all required uniforms
		//
		//					// draw
		//					if (useElements)
		//						glDrawElements(GL_TRIANGLES, meshGroups.usedMesh.get_indicies_size(), 
		...
		void scenegraph3d::__add(mesh_component* mc) {
			if (!mc || !mc->get_mesh().is_valid()) return;
